    ...
    public String checkout(final Principal principal, final Model model) {
        if (principal != null) {
            ...
            if (user != null) {
                ...
                for (final Orders orders : cart) {
                    discountedPrice += orders.getOrderPrice() * orders.getProductQuantities();
                    final Optional<Product> productOptional = this.productRepository.findById(orders.getProductName());

                    if (productOptional.isPresent()) {
                        orderPrice += productOptional.get().getOriginalPrice() * orders.getProductQuantities();
                    }
                }
